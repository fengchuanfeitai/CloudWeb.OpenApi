layui.use(['form', 'laypage', 'layer', 'table', 'element'], function () {
    ...
    //    , contentType: 'application/son'//传值格式
    ...
    layui.config({
        ...
        var insTb = treeTable.render({
            ...
            cols: [[
                { type: 'numbers' },
                { type: 'checkbox' },
                //{ field: 'columnId', title: '菜单名称' },
                { field: 'colName', title: '菜单地址' },
                { field: 'columnId', title: '权限标识' },
                { field: 'sort', title: '权限标识' },
                { field: 'parentId', title: '权限标识' }
            ]]
            , done: function (res, curr, count) {
                //如果是异步请求数据方式，res即为你接口返回的信息。
                //如果是直接赋值的方式，res即为：{data: [], count: 99} data为当前页数据、count为数据总长度
